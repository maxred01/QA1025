...
async def run_api_tests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Запуск pytest
    result = await execute_command(
        "pytest -s -v .\\Popov_Nikolay\\homework\\test_artmas_by\\test_artmas_homework_2.py",  # --alluredir=./results
        update
    )
...
🔸команда: "/aboutmyself" - Информация об этом боте
🔸команда: "/runapitests" - Тестирование API 
🔸команда: "/runuitests" - UI тесты
...
def main():
    application = Application.builder().token('8719637940:AAFvLXz4J0ZIQdR7m9RVwCqQTOEXqDk1JNU').build()

    handlers = [
        CommandHandler("run_all_tests", run_all_tests),
        CommandHandler("run_ui_tests", run_ui_tests),
        CommandHandler("runapitests", run_api_tests),
